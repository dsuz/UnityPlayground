...
public class LoadLevelActionInspector : InspectorBase
{
    ...
    private string explanation = "現在と同じシーンをリロードする、または指定したシーンを読み込む。File > Build Settings から Scenes In Build に追加したシーンを選択できる。";
    //private string sceneWarning = "WARNING: Make sure the scene is enabled in the Build Settings scenes list.";
    private string sceneWarning = "指定したシーンが File > Build Settings の Scenes In Build でチェックされていることを確認して下さい。";
    ...
    private string sceneInfo = "ロードできるシーンを追加するには、File > Build Settings から Scenes In Build にシーンを追加します。";
